...
describe("TowersOfHanoi Component (Refactored)", () => {
  // Reduce test overhead by cleaning up after each test only once
  afterEach(() => {
    // Unmounts the component and cleans up the DOM
    // This is handled by @testing-library/react v14+ automatically, but explicit for clarity
  ...
  it("when the start button is clicked, then the UI updates after each move", async () => {
    ...
    // Advance all timers at once for speed
    act(() => {
